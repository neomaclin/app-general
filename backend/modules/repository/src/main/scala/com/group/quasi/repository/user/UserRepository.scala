...
class UserRepository(implicit override val session: SlickSession)
  ...
  def insert(user: User): Future[Int] = {
    session.db.run(
      sqlu"""INSERT INTO users (id, alias, email, alias_lowercase, email_lowercase, phone, password, salt, node_creation_epoch, active) VALUES(${user.id}, ${user.login},${user.email},${user.login.toLowerCase},${user.email.toLowerCase}, ${user.phone},  ${user.password},  ${user.salt},${user.nodeTime}, false)""",
    )
  }
  ...
  class Users(tag: Tag) extends Table[User](tag, "users") {
    ...
    def * = (id, login, email, password,salt, phone.?, timeOnNode, active) <> (User.tupled, User.unapply)
  }
}
