...
import com.group.quasi.domain.model.{roles, users}
import com.group.quasi.domain.model.users.{ActivationFailure, ActivationSuccess, LoginSuccess, User}
...
class UserServiceImpl[F[_]: MonadThrow](
    ...
    val result: F[Either[Unit, String]] =
      MonadThrow[F].ifM(userRepo.findByLoginOrEmail(user, email).map(_.isEmpty))(
        for {
          ...
          _ <- userRepo.insert(User(id = userId, login = user, password = password, email = email, phone = phone, active = false))
  ...
  override def login(requestFrom: String,
                     user: String,
                     password: String,
                     email: Option[String],
                     phoneNumber: Option[String]): F[Either[users.LoginFailure, users.LoginSuccess]] = {
    val result = for {
      ...
      } else userOption.filter(_.password === password)
  ...
  override def lookup(user: users.SuccessContent): F[Either[Unit, Option[roles.Role]]] = {
    val result: F[Either[users.ActivationFailure, users.ActivationSuccess]] = for {
      userIdOption <- userRepo.findByLoginOrEmail().findByKey(key)
      userOption <- userIdOption.traverse(userId => userRepo.findById(userId))
      _ <- userOption.flatten.traverse(user => userRepo.activate(user.id))
      _ <- userOption.flatten.traverse(user => activationKeyRepository.delete(key, user.id))
    } yield {
