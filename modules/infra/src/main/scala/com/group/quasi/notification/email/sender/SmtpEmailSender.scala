package com.group.quasi.notification.email.sender
...
/** Sends emails synchronously using SMTP.
  */
class SmtpEmailSender[F[_]:Monad](config: SmtpConfig) extends NotificationSender[F] {

  def send(notification: NotificationData): F[Unit] = for {
    emailToSend <- new SmtpEmailSender.EmailDescription(List(notification.recipient), notification.content, notification.subject).pure[F]
  } yield {
